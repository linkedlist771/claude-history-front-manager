...
function addCustomStyles () {
  ...
  style.textContent = `
    ...
    /* Menu container and header styles */
    .menu-container {
      width: 100%;
    }
    ...
    html.dark .minimize-btn:hover {
      background: rgba(255, 255, 255, 0.1);
    }
    
    /* Minimized floating button styles */
    #menuButton.minimized {
...
function updateThemeStyles () {
  ...
  // Create menu button with fixed positioning
  ...
  // Create menu with fixed positioning
...
function createMenu() {
  ...
  if (isVisitor === 'false') {
    // Check if we should show the redemption button based on closeCardExchange
    ...
    menuItems = `<div class="menu-container">
      <!-- Menu header with minimize button -->
      <div class="menu-header">
        <span class="menu-title">功能菜单</span>
        <button class="minimize-btn" onclick="toggleMenuMinimize()">
          ${createMinimizeIcon()}
        </button>
      </div>
      
      <div class="flex flex-col space-y-1" style="padding: 8px;">
        <!-- User info section -->
        <div style="margin-bottom: 10px;">
          <!-- Redemption button - only show if closeCardExchange is undefined or 'false' -->
...
function initCustomUI () {
  ...
  // Define menu control functions globally
  window.toggleMenuMinimize = function() {
    menuState.isMinimized = !menuState.isMinimized;
    updateMenuDisplay();
  };

  window.toggleMenuVisibility = function() {
    if (menuState.isMinimized) {
      // If minimized, clicking should maximize and show
      menuState.isMinimized = false;
      menuState.isVisible = true;
    } else {
      // If not minimized, toggle visibility
      menuState.isVisible = !menuState.isVisible;
    }
    updateMenuDisplay();
  };
  ...
  window.updateMenuDisplay = function() {
    ...
    if (menuState.isMinimized) {
      ...
      menuButton.style.backgroundColor = document.documentElement.classList.contains('dark') ? '#333' : '#fff';
      menuButton.style.boxShadow = '0 2px 8px rgba(0,0,0,0.15)';
      menuButton.style.border = '1px solid ' + (document.documentElement.classList.contains('dark') ? '#555' : '#e5e5e5');
      menuButton.innerHTML = createMaximizeIcon();
      menuButton.classList.add('minimized');
    } else {
      // Normal state
      menuButton.style.right = '20px';
      menuButton.style.top = '10%';
      ...
      menuButton.innerHTML = createMenuIcon();
      menuButton.classList.remove('minimized');
...
function initRegAndLoginButton() {
  ...
  if (!navElement) {
    console.error('未找到nav元素，延迟500ms后重试');
    setTimeout(initRegAndLoginButton, 500);
    return;
  }

  const navDivs = navElement.querySelectorAll('div');
  if (navDivs.length === 0) {
    console.error('nav元素中没有找到div，延迟500ms后重试');
    setTimeout(initRegAndLoginButton, 500);
    return;
  }

  const lastDiv = navDivs[navDivs.length - 1];
  var $div = $(lastDiv);
...
function setupEventListeners() {
  ...
  $(document).on('click', function(e) {
    if (
      ...
      showMenu() && menuState.isVisible && !menuState.isMinimized
    ) {
      menuState.isVisible = false;
      window.updateMenuDisplay();
    }
  ...
  fetch(url)
    ...
    .then(({ code, data }) => {
      if (code === 200) {
        ...
        // 找到nav的最后一个div并重置初始化状态
        const navElement = document.querySelector('nav');
        if (navElement) {
          const navDivs = navElement.querySelectorAll('div');
          if (navDivs.length > 0) {
            const lastDiv = navDivs[navDivs.length - 1];
            $(lastDiv).removeClass('init');
          }
        }
